...
class MFLike(_InstallableLikelihood):
    ...
    def initialize(self):
        ...
        self.expected_params = ["a_tSZ", "a_kSZ", "a_p", "beta_p",
                                ...
                                "yp",
                                "cTT93","cTT145","cTT225",
                                "cEE93","cEE145","cEE225"] #Martina

        self.last_fg_index = 9 #Martina, this could be done better]
    ...
    def _get_systematic_template(self, sys_params):
        return get_systematic_template(sys_params=sys_params,
                                    sys_template=self.systematics,
                                    frequencies=self.freqs,
                                    ell=self.l_bpws,
                                    requested_cls=self.requested_cls)
...
def get_systematic_template(sys_params, sys_template,
                         frequencies, ell,
                         requested_cls=["tt", "te", "ee"]):#,cl):
    ...
    #    sys_params["cTT"+str(frequencies[1])],sys_params["cTT"+str(frequencies[2])]]
    #cal_pars["ee"]=[sys_params["cEE"+str(frequencies[0])],
    #    sys_params["cEE"+str(frequencies[1])],sys_params["cEE"+str(frequencies[2])]]
    cal_pars["tt"]=(1./sys_params["yp"])*np.array(1./np.sqrt([
        sys_params["cTT"+str(frequencies[0])],
        sys_params["cTT"+str(frequencies[1])],
        sys_params["cTT"+str(frequencies[2])]]))
    cal_pars["ee"]=(1./sys_params["yp"])*np.array(1./np.sqrt([
        sys_params["cEE"+str(frequencies[0])],
